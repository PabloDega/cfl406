...
function renderCursos(){
    ...
    cursos.forEach((curso) => {
        ...
        if(cierreInscripciones < new Date()){visibilidad = "oculto"}
        txtHTML += `<div class="card flexV">
                        <h2>Sede ${curso.sede}</h2>
                        <h1>${curso.curso}</h1>
                        <span>✔ Curso ${curso.tipo}</span>
                        <span>✔ Titulo ${curso.titulo}</span>
                        <span>X Inscripciones cerradas</span>
                        <div class="flexH">
                        <div class="btn btnInscripcion ${visibilidad}" data-id="${curso.id}">Inscripción</div>
